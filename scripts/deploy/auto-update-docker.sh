...
usage() {
    cat <<'EOF'
用法:
  bash scripts/deploy/auto-update-docker.sh [--version v4.5.55] [--with-release-assets] [--with-ghcr]
...
示例:
  ...
  export DOCKERHUB_TOKEN='***'
  export GHCR_TOKEN='***'
  bash scripts/deploy/auto-update-docker.sh --version 4.5.55 --with-ghcr --with-release-assets
EOF
}
