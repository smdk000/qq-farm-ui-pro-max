...
<template>
  <div class="space-y-4">
    ...
    <div v-else-if="items.length === 0" class="glass-text-muted border border-white/20 rounded-lg bg-black/5 p-8 text-center backdrop-blur-sm dark:border-white/10 dark:bg-black/20">
      无可展示物品
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-4 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 xl:grid-cols-6">
      <div
        v-for="item in items"
        :key="item.id"
        class="group glass-panel relative flex flex-col items-center border border-gray-200/50 rounded-lg p-3 transition dark:border-white/10 hover:shadow-md"
      >
