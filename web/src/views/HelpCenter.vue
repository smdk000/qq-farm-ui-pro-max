...
<template>
  <div class="glass-text-main h-full flex flex-col overflow-hidden bg-gray-50 p-4 dark:bg-[#0b1120] lg:p-8 sm:p-6">
    <div class="mx-auto h-full max-w-7xl w-full flex gap-6">
      <!-- 左侧分类导航 -->
      <aside class="spring-interactive h-full w-64 flex shrink-0 flex-col border border-gray-200/80 rounded-2xl bg-white/80 p-4 shadow-sm backdrop-blur-xl transition-all dark:border-white/5 dark:bg-white/[0.04] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
        ...
        <nav class="flex-1 overflow-y-auto pr-1 space-y-2">
          ...
          >
            <button
              ...
              :class="[
                category.expanded
                  ? 'bg-blue-50/80 border border-blue-200/50 dark:bg-white/[0.08] dark:border-white/10 text-blue-700 dark:text-cyan-300 font-bold shadow-[0_0_15px_rgba(59,130,246,0.1)]'
            ...
            >
              <div class="flex items-center gap-3">
                <div :class="[category.icon, category.expanded ? 'text-blue-600 dark:text-cyan-400' : 'glass-text-muted group-hover:scale-110 transition-transform']" />
                ...
                <button
                  ...
                  :class="[
                    selectedArticle === item.id
                      ? 'bg-blue-100 text-blue-700 font-bold dark:bg-white/[0.08] dark:text-cyan-300 shadow-sm'
                      : 'glass-text-muted hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-gray-200',
                  ]"
        ...
        <!-- 开发进度信息 -->
        <div class="mt-4 border border-gray-200 rounded-xl bg-white p-4 shadow-sm dark:border-white/5 dark:bg-black/40">
          <div class="glass-text-main mb-3 flex items-center gap-2 text-xs font-bold dark:text-gray-300">
            <div class="i-carbon-progress text-blue-600 dark:text-cyan-400" />
            <span>核心库补完计划</span>
          </div>
          <div class="h-1.5 overflow-hidden rounded-full bg-gray-200 dark:bg-white/10">
            <div
              class="h-full from-blue-500 to-indigo-600 bg-gradient-to-r transition-all duration-1000 ease-out dark:from-cyan-400 dark:to-blue-500"
      ...
      <main class="h-full min-w-0 flex flex-1 flex-col gap-6">
        <div class="relative min-h-0 flex flex-1 flex-col">
          <!-- 创作者与搜索卡片 (固定在顶部) -->
          <div class="group sticky top-0 z-20 mb-6 flex shrink-0 flex-col justify-between gap-4 overflow-hidden border border-gray-200/80 rounded-3xl bg-white/90 p-4 shadow-sm backdrop-blur-xl sm:flex-row sm:items-center dark:border-white/5 dark:bg-white/[0.04] lg:p-5 dark:shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
            <!-- 底部氛围光 -->
            <div class="pointer-events-none absolute h-64 w-64 rounded-full bg-blue-500/5 blur-[40px] transition-all duration-700 -bottom-10 -right-10 dark:bg-cyan-500/5 group-hover:bg-blue-500/10 dark:group-hover:bg-cyan-500/10" />
            ...
            <div class="relative z-10 flex items-center gap-4">
              <div class="spring-interactive relative h-12 w-12 rounded-full from-blue-500 to-indigo-600 bg-gradient-to-tr p-[2px] shadow-md transition-all duration-500 dark:from-cyan-400 dark:to-blue-500 group-hover:shadow-[0_0_15px_rgba(59,130,246,0.3)]">
                <div class="h-full w-full flex items-center justify-center border border-white/50 rounded-full bg-white dark:border-white/10 dark:bg-[#0b1120]">
                  <div class="i-carbon-code text-xl text-blue-700 dark:text-cyan-400" />
                </div>
              </div>
              <div>
            ...
            <div class="relative z-10 w-full flex items-center gap-3 sm:w-auto">
              <!-- 搜索框 -->
              <div class="spring-interactive flex flex-1 cursor-text items-center border border-gray-200/80 rounded-xl bg-gray-100/80 p-2.5 shadow-inner backdrop-blur-md transition-all sm:w-64 dark:border-white/5 group-hover:border-blue-400/50 dark:bg-black/50 dark:hover:border-white/10 dark:hover:shadow-[0_0_15px_rgba(34,211,238,0.1)]">
                <span class="glass-text-muted i-carbon-search ml-2 mr-2 text-lg transition-colors group-hover:text-blue-600 dark:group-hover:text-cyan-400" />
                <input
                  v-model="searchQuery"
                  type="text"
                  placeholder="搜索所有文档..."
              ...
              <!-- QQ群按钮 -->
              <a href="tencent://message/?uin=227916149&Site=&Menu=yes" class="spring-interactive flex items-center gap-2 whitespace-nowrap border border-gray-300 border-b-gray-400 rounded-xl bg-white px-4 py-2.5 text-sm text-gray-800 font-bold shadow-sm transition-all dark:border-white/10 hover:border-gray-400 dark:border-b-white/20 dark:bg-white/[0.06] dark:text-slate-100 hover:shadow-md dark:hover:border-white/30">
                <div class="i-carbon-chat text-lg text-blue-600 dark:text-cyan-400" />
          ...
          <!-- 文档内容卡片 -->
          <div class="relative flex-1 overflow-y-auto overscroll-contain scroll-smooth border border-gray-200/80 rounded-3xl bg-white/90 p-8 shadow-sm backdrop-blur-xl dark:border-white/5 dark:bg-white/[0.04] dark:shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
            <div class="pointer-events-none absolute right-0 top-0 h-32 w-full from-gray-50/50 to-transparent bg-gradient-to-b dark:from-white/[0.02]" />

            <!-- 搜索结果视图 -->
            <div v-if="isSearching" class="relative z-10">
              <h2 class="glass-text-main mb-6 flex items-center gap-3 text-2xl font-extrabold">
                <div class="i-carbon-search text-blue-600 dark:text-cyan-400" />
                搜索 "{{ searchQuery }}"
              </h2>
              <div v-if="searchResults.length > 0" class="space-y-3">
                <button
                  ...
                  :key="result.id"
                  class="spring-interactive group w-full flex items-start gap-4 border border-gray-200 rounded-xl bg-white p-4 text-left shadow-sm transition-all dark:border-white/5 hover:border-blue-400 dark:bg-white/[0.02] hover:bg-blue-50/80 hover:shadow-md dark:hover:border-white/20 dark:hover:bg-white/[0.06]"
                  @click="selectArticle(result.id)"
                >
                  <div class="h-10 w-10 flex shrink-0 items-center justify-center border border-gray-200 rounded-lg bg-gray-50 text-blue-600 shadow-sm dark:border-white/5 dark:bg-black/40 dark:text-cyan-400 group-hover:shadow-md">
                    <div :class="result.icon" class="text-xl" />
                  </div>
                  <div class="min-w-0 flex-1">
                    <h3 class="glass-text-main mb-1 text-base font-bold transition-colors group-hover:text-blue-700 dark:group-hover:text-cyan-300">
                      {{ result.title }}
                    </h3>
                    <div class="flex items-center gap-2">
                      <span class="glass-text-main rounded bg-gray-200 px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase dark:bg-white/10 dark:text-gray-300">{{ result.category }}</span>
                    </div>
                  </div>
                </button>
              </div>
            ...
            <div v-else class="relative z-10 max-w-3xl">
              <h2 class="glass-text-main mb-8 flex items-center gap-3 text-3xl font-black tracking-tight">
                <div class="i-carbon-document text-blue-600 dark:text-cyan-400" />
                {{ getArticleTitle() }}
              </h2>
              ...
              <div class="mt-12 flex flex-wrap items-center justify-between gap-4 border-t border-white/20 pt-6 dark:border-white/10/50">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex gap-2">
                    <button class="spring-interactive glass-text-muted h-9 w-9 flex items-center justify-center border border-transparent rounded-full bg-gray-100 shadow-sm transition-all hover:border-primary-300 dark:bg-white/[0.06] hover:bg-primary-100 hover:text-primary-700 dark:hover:border-primary-700/50 dark:hover:bg-primary-900/40 dark:hover:text-primary-400">
...
:deep(.article-markdown-body) {
  font-size: 0.95rem;
  color: #1f2937; /* text-gray-800 in light mode */
}
.dark :deep(.article-markdown-body) {
  color: #e5e7eb; /* text-gray-200 in dark mode */
}
