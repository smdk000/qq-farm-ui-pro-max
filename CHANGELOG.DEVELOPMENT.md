...
- `v4.5.62 (2026-04-18)` QQ 客户端版本 `1.10.0.13` 与双机同步升级版：默认 `clientVersion` 已切到 `1.10.0.13`，并统一主程序、部署模板、Docker 发布脚本和 Release 巡检口径到 `v4.5.62`，用于 GitHub、Docker 与双机多实例同步升级。
- `v4.5.61 (2026-04-02)` 田园 UI 正式发布与双机补发版：把本地已完成但未进入正式 Release 的田园 UI 入口、全屏工作台、快捷抽屉和场景资源正式纳入版本控制，并统一默认镜像、部署模板、工作流与 Release 巡检口径到 `v4.5.61`。
...
### 开发补记 - v4.5.62 QQ 客户端版本 1.10.0.13 与双机同步升级版 (2026-04-18)

#### ✅ 本轮 QQ 版本口径升级
- ✅ **默认 `clientVersion` 已从旧口径切到 `1.10.0.13`**: `core/src/config/config.js` 现在默认使用 `1.10.0.13`，用于应对服务端“客户端版本过低，请升级到最新版本”的强制踢下线。
- ✅ **`FARM_CLIENT_VERSION` 覆盖能力继续保留**: 仍可通过环境变量临时验证其他版本，不会阻断后续排障或按服务器差异做灰度。

#### ✅ 本轮发布与部署版本统一
- ✅ **当前默认发布版本已统一切到 `v4.5.62`**: `core/package.json`、`web/package.json`、部署模板、离线包脚本、Docker 发布脚本、远端更新脚本和 GitHub Actions 默认版本都已同步抬升。
- ✅ **Release 附件巡检默认范围已扩到 `v4.5.62`**: `check-release-assets.js` 现在会默认检查 `v4.5.54 ~ v4.5.62`。

#### ✅ 本轮用于双机多实例同步升级
- ✅ **本轮版本将作为 GitHub / Docker / 服务器结果的统一追踪基线**: 后续提交、镜像与双机验收都以 `v4.5.62` 对齐。
- ✅ **双机多实例升级口径已提前对齐**: `10.31.1.254` 与 `10.31.2.242` 上的 `qq-farm / qq-farm-2400 / qq-farm-2500 / qq-farm-2600` 都可以直接按这条版本线滚动更新。

### 开发补记 - v4.5.61 田园 UI 正式发布与双机补发版 (2026-04-02)

#### ✅ 本轮田园 UI 正式入库
- ✅ **`田园UI` 入口和 `/pastoral-view` 路由已进入正式发布仓**: 这次不再停留在本地未提交工作区，控制台顶部浮动入口、田园全屏页和返回控制台记忆链路已经进入干净源码。
- ✅ **田园工作台主体与场景资源已完整纳入**: `PastoralView.vue`、`PastoralFullscreenPage.vue`、`PastoralQuickPanel.vue`、`pastoral-view.ts` 和 `web/public/dashboard-assets` 已经作为正式版本内容收口。
- ✅ **`PastoralView.vue` 尾部异常空字节已清理**: 复制进干净发布仓时同步去掉了末尾的 NUL 填充，避免正式构建把页面文件判成异常文本。

#### ✅ 本轮版本与发布口径
- ✅ **当前默认版本已统一切到 `v4.5.61`**: `core/package.json`、`web/package.json`、部署模板、离线包脚本、Docker 发布脚本、远端更新脚本、GitHub Actions 默认版本和 README 示例命令都已同步抬升。
- ✅ **Release 附件巡检默认范围已扩到 `v4.5.61`**: `check-release-assets.js` 现在会默认检查 `v4.5.54 ~ v4.5.61`。
- ✅ **这轮版本用于修复“本地页面和正式版不一致”**: 后续 GitHub Release、Docker 镜像、README、帮助中心发布索引和双机服务器部署结果都会统一追踪到 `v4.5.61`。
