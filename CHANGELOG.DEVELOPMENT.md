...
- `v4.5.59 (2026-03-31)` 批量作业与发布部署闭环复核版：把 `v4.5.56 ~ v4.5.58` 的设置总控、农场工作台、批量作业反馈与 QQ 高风险保存预览统一收口到新的滚动版本，并继续承接 GitHub Release、Docker Hub、更新公告与双机部署复核。
...
### 开发补记 - v4.5.59 批量作业与发布部署闭环复核版 (2026-03-31)

#### ✅ 本轮版本收口目标
- ✅ **把最近三轮大改统一收口成新的滚动发布版本**: `v4.5.56` 的设置总控工作台收口、`v4.5.57` 的农场土地工作台与单地块操作，以及 `v4.5.58` 的批量作业反馈与 QQ 高风险保存预览，现已整理为一套稳定的对外版本摘要。
- ✅ **这次版本更偏向“发布基线复核”而不是单点功能追加**: 核心目的是让最近连续几轮真实改动可以用同一个版本号被 GitHub Release、Docker 镜像、更新公告和服务器安装流程稳定引用。

#### ✅ 本轮版本与公告统一
- ✅ **当前默认版本已统一切到 `v4.5.59`**: `core/package.json`、`web/package.json`、README、部署模板、Docker 发布脚本、GitHub Actions 默认版本与帮助中心 Release Notes 已同步抬升。
- ✅ **版本索引、帮助中心和运行态公告会继续共源**: `CHANGELOG.md`、`logs/development/Update.log`、README 最近更新区块和帮助中心 Release Notes 将继续围绕同一份版本索引同步，减少说明口径漂移。

#### ✅ 本轮发布与部署主线
- ✅ **当前版本会继续承接 GitHub Release / Docker Hub / GHCR 发布**: 作为新的对外版本，后续镜像标签、Release 资产和服务器升级示例默认都应指向 `v4.5.59`。
- ✅ **真实服务器仍是正式验收路径**: 本轮会继续覆盖 `10.31.1.254` 的最新程序安装与一键安装脚本测试，以及 `10.31.2.242` 的 `2400 / 2500 / 2600` 多实例同步升级。
- ✅ **历史 Release 缺口会继续补齐并复核**: 最近几个版本的 GitHub Release 将继续检查缺失 tag / 资产，确保安装、离线包和回滚链路完整可用。
