...
- `v4.5.59 (2026-03-31)` 批量作业与发布部署闭环复核版：汇总 `v4.5.56 ~ v4.5.58` 的设置总控、农场工作台、批量作业反馈与 QQ 高风险保存预览更新，统一 GitHub Release、Docker Hub、部署文档与更新公告到 `v4.5.59`，并继续复核双机多端口安装升级与历史 Release 完整性。
...
### 🧭 近期功能更新统一收口
- **把最近三轮大改整理成同一套对外发布口径**: `v4.5.56` 的设置总控工作台收口、`v4.5.57` 的农场土地工作台与单地块操作，以及 `v4.5.58` 的批量作业反馈与 QQ 高风险保存预览，现统一汇总为新的滚动版本，避免真实能力和外部版本说明继续脱节。
- **本轮更适合作为“发版 + 部署”共同基线**: 这次不是回退已有功能，而是把最近连续多轮改动重新收束到一个稳定版本，方便 GitHub Release、Docker 镜像和服务器安装升级一起对齐。

### 📦 发布口径统一抬升到 `v4.5.59`
- **默认版本入口统一切到 `v4.5.59`**: `core/package.json`、`web/package.json`、README、部署模板、Docker 发布脚本、GitHub Actions 默认版本与帮助中心 Release Notes 已统一切到 `v4.5.59`。
- **更新公告与部署文档继续保持共源**: `CHANGELOG.md`、`logs/development/Update.log`、README 最近更新区块和帮助中心 Release Notes 会继续围绕同一份版本摘要同步，减少“代码已经更新，但公告和安装示例仍停留在旧版”的情况。

### 🚀 GitHub / Docker / 双机部署继续闭环
- **当前版本将继续承接 GitHub Release、Docker Hub / GHCR 与更新公告发布**: 后续镜像、Release 资产和服务器升级示例默认都应指向 `v4.5.59`。
- **真实服务器继续作为正式验证路径**: 本轮会继续覆盖 `10.31.1.254` 的最新程序安装/更新与一键安装脚本测试，以及 `10.31.2.242` 的 `2400 / 2500 / 2600` 多实例同步升级。

### ✅ 历史 Releases 完整性复核继续推进
- **最近几个正式版本的 GitHub Releases 会继续补齐缺失 tag / 附件并复核完整性**: 确保当前版本和最近几个历史版本都能完成 Release 资产下载、离线部署、源码回放和回滚定位。
